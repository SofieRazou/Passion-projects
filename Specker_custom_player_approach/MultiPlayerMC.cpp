...
public:
    ...
    void play(std::ofstream& results, int game_number) {
        while (!state->winning()) {
            Player* p = players[state->getPlaying()];
            Move m = p->play(*state);
            state->next(m);
        }
        unsigned int winner = state->getPlaying();  // last player to *not* move
        results << game_number << "," << players[winner]->getName() << "\n";
    }
...
// run this if you want to run a sum of games otherwise comment it 
// --- Main ---
int main() {
    srand(time(0));

    const int NUM_RUNS = 100;
    std::ofstream results("game_results.txt");
    results << "Game,Winner\n";
