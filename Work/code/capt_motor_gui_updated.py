...
class SpringWidget(QWidget):
    ...
    def set_reference_angle(self, reference_rad: float) -> None:
        self.reference_angle = reference_rad
        self.update()
    ...
    def paintEvent(self, event) -> None:
        ...
        normalized_angle = max(
            -1.0,
            min(1.0, self.angle / maximum_angle),
        )
        ...
        reference_deg = math.degrees(self.reference_angle)

        painter.drawText(
            20,
            30,
            f"Measured angle: {angle:.2f}°",
        )
        painter.drawText(
            20,
            55,
...
class SpringPage(QWidget):
    ...
    def update_measurements(
        ...
        self.spring_view.set_angle(angle)
        self.spring_view.set_measured_torque(torque)

        self.angle_label.setText(
            f"Measured angle: {math.degrees(angle):.3f}°"
        )
